...
class EarningsController extends Controller
{
    ...
    {
        $keys = collect($this->environment())->keys()
            ->toArray();
    ...
    public function payout(Request $request)
    {
        ...
        if (!$wallet) {
            return error_response($request, __('Selected escrow wallet could not be found!'));
        }

        if (!$wallet->balance > 0) {
            return error_response($request, __('Selected escrow wallet does not have sufficient balance!'));
        }
