...
use App\Models\KomodoAddress;
use App\Models\KomodoTransaction;
...
class KomodoAdapter
{
    ...
    public function generateWallet($label, $passphrase, $userid, $username)
    {
        ...
        $generate_wallet->post($api_url.'createWallet',array('type'=>1,'coin'=>'KMD','userid'=>$userid,'username'=>$username,'passphrase'=>$passphrase));
    ...
    public function generateEscrowWallet($label, $passphrase)
    {
        ...
        $generate_wallet->setHeader("Authorization",$this->accesstoken);
        $api_url = $this->api_url;
        $generate_wallet->post($api_url.'createWallet',array('type'=>2,'coin'=>'KMD','passphrase'=>$passphrase));
        if($generate_wallet->errorMessage){
            throw new BlockchainException(__('Unable to generate wallet'));
            //throw new BlockchainException(__(json_encode($generate_wallet->response)));
        }
    ...
    public function sendMultiple($wallet, $outputs)
    {
        ...
        $api_url = $this->api_url;
        $send_multiple->post($api_url.'transaction',array('coin'=>'KMD','userid'=>$wallet->user_id,'wallet_id'=>$wallet->wallet_id,'wallet_key'=>$wallet->passphrase,'addresses'=>$addresses,'amounts' =>$amounts));
    ...
    public function send($wallet, $output, $amount)
    {
        if ($amount < 0) {
            ...
            $api_url = $this->api_url;
            $send_tx->post($api_url.'withdraw',array('coin'=>'KMD','userid'=>$wallet->user_id,'wallet_id'=>$wallet->wallet_id,'wallet_key'=>$wallet->passphrase,'address'=>$output,'amount'=>$amount));
    ...
    {
        if (!is_array($output)) {
            $address = KomodoAddress::where('address', $output);

            $address->first()->wallet->increment('balance', $amount);
        } else {
            foreach ($output as $out) {
                $address = KomodoAddress::where('address', $out['address']);
