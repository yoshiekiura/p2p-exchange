            ...
            <ul class="nav nav-tabs">
                <li class="nav-item">
                    <a class="nav-link active text-bold-600" id="komdo-address-tab" aria-expanded="true"
                       href="#ripple-address" aria-controls="ripple-address" data-toggle="tab">
                        <i class="ft-hash"></i> {{__('ADDRESSES')}}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-bold-600" id="ripple-transaction-tab" href="#ripple-transaction"
                       aria-controls="ripple-transaction" aria-expanded="false" data-toggle="tab">
                        <i class="ft-activity"></i> {{__('TRANSACTION')}}</a>
                </li>
            </ul>
            <div class="tab-content px-1 pt-1">
                <div role="tabpanel" class="tab-pane active" id="ripple-address"
                     aria-labelledby="ripple-address-tab" aria-expanded="true">

                    @if(Auth::user()->wallet('xrp')->latestAddress())
                        <div class="row">
                            <div class="col-sm-6 py-1 text-center">
                                {!! HTML::image(Auth::user()->wallet('xrp')->latestAddressQRCode(), null, ['class' => 'img-thumbnail']) !!}
                            </div>
                            <div class="col-sm-6 py-4 text-center">
                                <div class="card-text size-2x text-uppercase">
                                    {{__('Your ripple deposit address is below:')}}
                                </div>
                                <div class="card-text text-center pt-1">
                                    <h2><b>{{Auth::user()->wallet('xrp')->latestAddress()}}</b></h2>
                                </div>

                                <div class="card-text text-center pt-1">
                                    <button type="button" class="btn mr-1 mb-1 btn-success"
                                            data-toggle="modal" data-target="#send-ripple">
                                        <i class="la la-send"></i> {{__('SEND')}}
                                    </button>
                                </div>

                                <div class="modal fade text-left" id="send-ripple" tabindex="-1" role="dialog" aria-labelledby="send-ripple-label"
                                     aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        {!! Form::open(['url' => route('wallet.send', ['coin' => 'xrp']), 'class' => 'form form-horizontal', 'method' => 'POST']) !!}
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4 class="modal-title" id="send-ripple-label">
                                                    <i class="la la-send"></i> {{__('Send')}} Ripple</h4>
                                            ...
                                            <div class="modal-body">
                                                <div class="form-body">
                                                    <h3 class="text-center">
                                                        {{__('Available:')}} {{Auth::user()->wallet('xrp')->totalAvailable()}} XRP
                                                    </h3>
                                                    <hr>
                                                    <div class="form-group row">
                                                        <label class="col-md-4">{{__('Value')}}</label>
                                                        <div class="col-md-8">
                                                            {!! Form::text('amount', null, ['class' => 'form-control', 'v-model.number' => 'send.xrp_value', 'placeholder' => Auth::user()->wallet('xrp')->totalAvailable(), 'novalidate']) !!}
                        ...
                        <div class="row pt-1">
                            <div class="table-responsive">
                                <table id="ripple-address-list" class="table table-white-space table-bordered row-grouping display icheck table-middle">
                                    <thead>
                                    <tr>
                                        <th></th>
                    ...
                    <div class="row pt-1">
                        <button class="btn btn-secondary mx-auto col-lg-8 btn-lg my-1 btn-block"
                                ...
                                type="button" href="{{route('wallet.generate-address', ['coin' => 'xrp'])}}">
                            {{__('Generate New Address')}}
                        </button>
                    </div>
                </div>
                <div class="tab-pane" id="ripple-transaction" role="tabpanel"
                     aria-labelledby="ripple-transaction-tab" aria-expanded="false">
                    <div class="card-text">
                    ...
                    <div class="row pt-1">
                        <div class="table-responsive">
                            <table id="ripple-transaction-list" class="table table-white-space table-bordered row-grouping display icheck table-middle">
                                <thead>
                                <tr>
                                    <th></th>
